...
from accidentes.PKG2_Respuesta_Emergencias.CU22_Gestionar_Unidades_Emergencia.repositories import (
...
from accidentes.shared.catalogo_repositories import TipoUnidadCatalogoRepository
...
class UnidadEmergenciaGestionService:
    ...
    @staticmethod
    def listar_unidades(tipo: str = None, estado: str = None, activo: bool = None, search: str = None) -> List[Dict[str, Any]]:
        filas = UnidadEmergenciaReadRepository.get_all(activo=activo)
        for u in filas:
            u.setdefault("estadounidad", "En base")
        if tipo:
            filas = [u for u in filas if u.get("tipounidademergencia") == tipo]
        if estado:
            filas = [u for u in filas if u.get("estadounidad") == estado]
        if search:
            search_lower = search.lower()
            filas = [u for u in filas if search_lower in u.get("unidademergencia", "").lower()]
        return filas

    @staticmethod
    def _tipo_id_a_nombre(tipo_id: int) -> str:
        nombre = TipoUnidadCatalogoRepository.get_nombre(tipo_id)
        if not nombre:
            raise ValueError(f"ID de tipo de unidad inválido: {tipo_id}")
        return nombre

    @staticmethod
    def crear_unidad(nombre: str, tipo_id: int) -> Dict[str, Any]:
        tipo = UnidadEmergenciaGestionService._tipo_id_a_nombre(tipo_id)
    ...
    @staticmethod
    def actualizar_unidad(unidad_id: int, nombre: str, tipo_id: int) -> Dict[str, Any]:
        tipo = UnidadEmergenciaGestionService._tipo_id_a_nombre(tipo_id)
